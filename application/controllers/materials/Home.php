...
class Home extends CI_Controller {
    ...
    */
    function __construct()
    {
        ...
        //load models
        $this->load->model(array('materials/material_model','urn_model','document/document_model'));
    }
    ...
    */
    ...
    function listRecords($page = 0)
    {
        ...
        ********************************************************/
        ...
        //materials
        $material           = $this->material_model->material();
        $data['material']   = $material;
        $records            = $this->material_model->getAllrecords($starting,$recpage,FALSE);
        $rec_total          = $this->material_model->getAllrecords($starting,$recpage,TRUE);
        if($records)
        {
        ...
        $this->ajax_pagination_new->make_search(
            ...
            base_url()."index.php/materials/home/listRecords",
            'list_div1',
            $str_post_str
        );
        ...
        $data['filter']         = $this->load->view("filter/material_filter",$data,true); 
        if($this->input->post('ajax')==1)
        {
            $data["search"] = TRUE;
            $this->load->view('filter/material_filter_list',$data);
        }else{                                                   
            banner();
            sidebar();
            $modal = modal_popup();
            $content = $this->load->view('materials/list',$data,true);
            content($content);
            footer();
        }
    }
    ...
    */
    function add($queue_urn = 0)
    {
        if(0){
            ...
            $this->form_validation->set_rules('m_name[]', 'dr_name[]', 'trim');
            if($this->form_validation->run() == FALSE){
                ...
                sidebar();
                $modal = modal_popup();
                $content = $this->load->view("materials/add",$data,true);
                content($content);
                footer();
            }
            else{
                ...
                foreach($m_name AS $key=>$value){
                    if($m_name[$key] != ''){
                        ...
                        $m_urn = $this->urn_model->getURN('materials','urn');
                        $updateDrug = $this->material_model->update('materials',$m_urn,$material_data,1000001,"INSERT");
                    }
                }
                if($updateDrug){
                    redirect("materials/home/listRecords",'refresh');
                }     
            }
        }
    }
    ...
    */
    function edit($urn = 0)
    {
        if(0){
            ...
            $this->form_validation->set_rules('m_name', 'm_name', 'trim');
            if($this->form_validation->run() == FALSE){
                $data = "";
                $records = false;
                //top title
                $data['title'] = $this->lang->line("register_edit");
                $records = $this->material_model->getViewRecords($dec_urn);
                ...
                $content = $this->load->view("materials/edit",$data,true);
                content($content);
                footer(); 
            }
            else{
                ...
                $updateDrug = $this->material_model->update('materials',$dec_urn,$drug_data,1000001,"UPDATE");
                if($updateDrug){
                    redirect("materials/home/listRecords/$urn",'refresh');
                }     
            }
        }
    }
                            ...
                            <tr> 
                                <td scope=\"col\" width=\"33%\" class=\"iEntry\">
                                    <div class=\"inputfield\">
                                        <div class=\"rLabel\">
                                            <label class=\"\" for=\"textinput\">$m_name : </label>                
                                        </div>
                                        <div class=\"textfield btm20padding\">
                                            <input id=\"m_name[]\" name=\"m_name[]\" type=\"text\" placeholder=\"$m_name\" class=\"form-control iInput\" >     
                                        </div>
                                    </div>
                                </td>
                            </tr>
    ...
    */
    function filter()
    {
        ...
        {           
            ...
            $records   = $this->material_model->search_records($starting,$recpage,FALSE);
            $rec_total = $this->material_model->search_records($starting,$recpage,TRUE);
            if($records)
            {
                //echo "<pre/>"; print_r($records->result()); exit;
                $data['records'] = $records;
            ...
            $this->ajax_pagination_new->make_search(
                ...
                $this->lang->line('of'),
                $this->lang->line('total'),
                base_url()."index.php/materials/home/filter",
                'list_div1',
                $str_post_str
            );
