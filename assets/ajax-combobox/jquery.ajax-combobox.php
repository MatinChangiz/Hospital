...
$mysql = array(
	'dsn'      => 'mysql:host=localhost;dbname=uni;charset=utf8',
	'username' => 'root',
	'password' => 'root'
);
$sqlite = array(
...
new AjaxComboBox($mysql, ( isset($_GET['w']) ? $_GET['w'] : '1' ) );// rezawm
//++++++++++++++++++++++++++++++++++++++++++++++++++++
...
class AjaxComboBox {
	...
	var $bind  = null;
	var $query = null;
		// rezawm following line added by me
	var $where = null;
	 ...
	 */
	function __construct($connect, $w) {
		// rezawm following line added by me
		$this->where = $w;
	...
	function makeSqlAll() {
		return array(
			...
			// 件数取得用
			sprintf("SELECT COUNT(*) FROM {$this->param['db_table']} where (%s)", $this->where)
		);
	}
	...
	function makeSqlSuggest() {
		...
		// rezawm added the following line
		if($this->where!='')
			$this->param['where'] = sprintf("%s and %s", $this->param['where'], $this->where);
//		die($this->param['where']);
